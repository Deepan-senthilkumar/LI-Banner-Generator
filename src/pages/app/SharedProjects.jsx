...
import { Share2, Loader2, Search, ExternalLink, AlertCircle } from 'lucide-react';
...
const SharedProjects = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto">
        ...
        {error ? (
          <div className="mb-4 p-3 rounded-xl bg-red-50 text-red-700 text-sm inline-flex items-center gap-2">
            <AlertCircle size={16} />
            {error}
          </div>
        ) : null}
        ...
        <div className="mb-5 relative max-w-sm">
          ...
          <input
            ...
            placeholder="Search title or teammate"
            className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white"
          />
        </div>
        ...
        {filteredProjects.length === 0 ? (
          <div className="text-center py-20 bg-slate-50 dark:bg-slate-900 rounded-3xl border border-dashed border-slate-300 dark:border-slate-800">
              ...
              <p className="text-slate-500 mt-2">When a teammate shares projects, they will appear here.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredProjects.map((project) => (
              <div key={project.id} className="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                <div className="aspect-[4/1] bg-slate-100 dark:bg-slate-800 overflow-hidden">
                  {project.previewImage ? (
                    <img
                      ...
                      alt={project.title || 'Shared project'}
                ...
                <div className="p-4">
                  <h3 className="font-semibold dark:text-white truncate" title={project.title}>
